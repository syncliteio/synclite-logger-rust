...
use synclite_db_traits::{Row, Value};

use crate::{default_config_for_backend, Logger};
use crate::sql_split::split_sqls;

const DEFAULT_BATCH_CAPACITY: usize = 4096;

/// SyncLite-wrapped DuckDB connection with a `duckdb`-shaped surface.
...
impl Connection {
    ...
    /// Initialize a SyncLite-managed DuckDB database using path-derived defaults.
    pub fn initialize<P: AsRef<Path>>(db_path: P) -> Result<Self> {
        Logger::initialize(DeviceType::DuckDb, db_path.as_ref())?;
        Self::open(db_path)
    }

    /// Open a SyncLite-managed DuckDB database using path-derived defaults.
    pub fn open<P: AsRef<Path>>(db_path: P) -> Result<Self> {
        let cfg = default_config_for_backend(db_path.as_ref().to_path_buf(), Backend::DuckDb);
        Ok(Self {
    ...
    /// Initialize from an existing `synclite_logger.conf` file.
    pub fn initialize_with_config<P: AsRef<Path>>(conf_path: P) -> Result<Self> {
        let cfg = synclite_config::SyncLiteConfig::load(&conf_path)?;
        let db_path = cfg.db_path.clone().ok_or_else(|| {
            synclite_core::Error::Config(
                "db-path is required in config for initialize_with_config".to_string(),
            )
        })?;
        Logger::initialize_with_config(DeviceType::DuckDb, db_path, cfg)?;
        Self::open_with_config(conf_path)
    }

    /// Open from an existing `synclite_logger.conf` file.
