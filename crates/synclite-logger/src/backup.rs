...
use synclite_core::{Backend, Error, Result};
...
pub fn bootstrap_synclite_txn_table(db_path: &Path, backend: Backend) -> Result<()> {
    match backend {
...
pub fn run_initial_backup_if_needed(
    ...
    let backup_shipped = metadata.get_i64("backup_shipped")?.unwrap_or(0);
    if backup_taken == 1 && backup_shipped == 1 {
        return Ok(());
    }

    std::fs::create_dir_all(&archive.stage_subdir)?;

    // 1. Take the local backup via schema+row copy.
    if backup_taken != 1 {
        if !cfg.use_precreated_data_backup.unwrap_or(false) {
            take_backup(&layout.db_path, &layout.backup_local_path, backend, cfg)?;
        }
        metadata.put_i64("backup_taken", 1)?;
    }

    // 2. Copy the backup into the stage subdir.
    ...
    // 4. Delete the local backup snapshot after shipping.
    if layout.backup_local_path.exists() {
        if let Err(e) = std::fs::remove_file(&layout.backup_local_path) {
            tracing::warn!(
                path = %layout.backup_local_path.display(),
                error = %e,
                "failed to remove local backup after staging"
            );
        }
    }
...
fn take_backup(src: &Path, dst: &Path, backend: Backend, cfg: &SyncLiteConfig) -> Result<()> {
    ...
    let copied_any_tables = match backend {
