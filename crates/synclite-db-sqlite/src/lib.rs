//! SQLite-backed [`DbDevice`](synclite_db_traits::DbDevice) implementation.
//!
...
use synclite_db_traits::{DbDevice, Row, TxnLogRuntime, Value};
use synclite_log::{
    scan_segment_dir,
    LogSegmentWriter,
...
///
/// Used to plug in a shipper without forcing every consumer to depend on
/// the shipper crate.
...
pub struct SqliteDeviceConfig {
    ...
    /// Optional directory to scan for resume state. When `None`, the
    /// device scans `segment_dir`. Useful when finalized segments are
    /// moved out of `segment_dir` (e.g. into a stage subdirectory) so
    /// resume must consult the post-move location instead.
...
/// Per-transaction stage file for concurrent-writer parity in STREAMING mode.
struct TxnStage {
    commit_id: u64,
    path: PathBuf,
    log: LogSegmentWriter,
...
impl SqliteDevice {
    ...
    pub fn roll_segment(&mut self) -> Result<()> {
        ...
        let should_ship = !old_log.is_empty()?;
        if should_ship {
            old_log.finalize()?;
        }
        drop(old_log);
    ...
    fn queue_record(&mut self, sql: Option<&str>, params: &[Value]) -> Result<()> {
        if !self.txn_runtime.is_open() {
            ...
            if self.cfg.device_type.is_streaming() {
                // STREAMING mirrors Java multiwriter behavior: BEGIN in the
                // main segment, txn body in a staged txn file.
...
impl DbDevice for SqliteDevice {
    ...
    fn execute(&mut self, sql: &str, params: &[Value]) -> Result<u64> {
        ...
        if self.cfg.device_type.is_streaming() {
            let op = self.next_operation_id.saturating_sub(1);
            self.update_synclite_txn(self.next_commit_id, op)?;
        }
        Ok(affected)
    }
    ...
    fn execute_prepared_batch(&mut self, sql: &str, batch_params: &[Vec<Value>]) -> Result<Vec<u64>> {
        ...
        let batch_len = batch_params.len() as u64;
        let first_logged_operation_id = self.next_operation_id.saturating_sub(batch_len);
        let mut out = Vec::with_capacity(batch_params.len());
        let mut streaming_ops = Vec::new();

        {
            ...
            for (idx, params) in batch_params.iter().enumerate() {
                ...
                if device_type.is_streaming() {
                    let op = first_logged_operation_id + idx as u64;
                    streaming_ops.push(op);
                }
                out.push(affected);
            }
        }
    ...
    fn close(self: Box<Self>) -> Result<()> {
        ...
        if me.cfg.device_type.is_transactional() {
            if me.cfg.device_type.is_streaming() {
                if me.txn_runtime.is_open() {
                    me.txn_runtime.close_with_tail(None);
                    me.flush_txn_records(false, false)?;
                    me.append_record(Some("ROLLBACK"), &[])?;
                }
            } else {
