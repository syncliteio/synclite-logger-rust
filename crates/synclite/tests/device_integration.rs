...
/// `~/synclite/tests/` for Rust device integration tests. Per-device
/// directories are created under `db/rustologger/<device>`; the stage and
/// work directories are shared with other project test suites under
/// `tests/stageDir` and `tests/workDir`.
fn test_home() -> PathBuf {
    let home = std::env::var_os("USERPROFILE")
        .or_else(|| std::env::var_os("HOME"))
        .map(PathBuf::from)
        .unwrap_or_else(std::env::temp_dir);
    home.join("synclite").join("tests")
}
...
impl Workspace {
    fn new(test_name: &str, device_name: &str, engine: &str, with_target: bool) -> Self {
        ...
        // Match the Java test layout under ~/synclite/tests/db/rustologger/<TestName>/
...
fn sqlite_sql_device_end_to_end_consolidates_into_destination_sqlite() {
    ...
    assert_eq!(
        update_rows,
        vec![
            (11, "a".to_string(), 1, "a".to_string()),
            (12, "b".to_string(), 2, "b".to_string()),
            (11, "a".to_string(), 1, "a".to_string()),
            (12, "b".to_string(), 2, "b".to_string()),
        ],
...
/// `~/synclite/tests/workDir`), so config side-effects cannot leak.
