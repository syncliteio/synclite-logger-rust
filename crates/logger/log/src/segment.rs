...
impl LogSegmentWriter {
    ...
    /// Remove all commandlog rows for a specific commit id.
    ///
    /// Used to discard an incomplete tail transaction before close so a
    /// segment never ships with a commit id that has no terminal fate.
    pub fn delete_commit_rows(&self, commit_id: u64) -> Result<()> {
        let inner = self.inner.lock();
        if inner.finalized {
            return Err(Error::Log("segment already finalized".into()));
        }
    ...
    /// Whether the latest commit id in this segment has a terminal fate
    /// (`COMMIT` or `ROLLBACK`) recorded.
    pub fn last_commit_fate_decided(&self) -> Result<bool> {
        let inner = self.inner.lock();
        let latest_commit: Option<i64> = inner
            .conn
    ...
    /// Whether the provided commit id already has a terminal fate marker.
    pub fn commit_fate_decided(&self, commit_id: u64) -> Result<bool> {
        let inner = self.inner.lock();
        let fate_count: i64 = inner
            .conn
            .query_row(
    ...
    /// Finalize without adding an automatic COMMIT marker.
    /// Used for txn stage files which should only contain user records.
    pub fn finalize_without_commit_marker(&self) -> Result<()> {
        self.finalize_internal(false)
    }

    fn finalize_internal(&self, add_commit_marker: bool) -> Result<()> {
        let mut inner = self.inner.lock();
        if inner.finalized {
            return Ok(());
        }
        ...
        // Do not synthesize COMMIT rows during finalize; COMMIT must be logged
        // only by actual committed transaction flow.
        let _ = add_commit_marker;
...
mod tests {
    use super::*;
    use logger_core::{CommitId, OperationId};
