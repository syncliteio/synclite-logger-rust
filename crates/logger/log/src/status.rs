...
pub enum LogSegmentStatus {
    ...
    ReadyToApply,
    /// Fully applied to every configured destination. Java parity:
    /// `LogSegment.markApplied()` writes this string into the segment's
    /// `metadata.status` row. The segment file is intentionally kept on
    /// disk (only segments < N are cleaned up) so callers can poll for
    /// drain completion deterministically.
    Applied,
}
...
impl LogSegmentStatus {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            LogSegmentStatus::Applied => "APPLIED",
        }
    }
}
