...
fn worker_loop(rx: mpsc::Receiver<Msg>, cfg: ShipperConfig) {
    ...
    loop {
        ...
        while (next as u64) <= latest_seq {
            ...
            // Only ship segments that have been finalized
            // (status = READY_TO_APPLY). An open writer may still hold the
            // file with status NEW; shipping it would race the writer.
            if !is_segment_ready(&segment) {
                break;
            }
...
fn is_segment_ready(path: &Path) -> bool {
    // Skip shipping only when we can definitively read status='NEW'
    // (i.e., the writer hasn't finalized yet). For any other case —
    // missing metadata table, unreadable file, or status not NEW — allow
    // the archiver to handle it. Synthetic test fixtures without the
    // metadata table thus still ship.
    ...
    let status: std::result::Result<String, rusqlite::Error> = conn.query_row(
...
mod tests {
    ...
    use std::sync::Mutex;
    use logger_archiver::FsArchiver;
